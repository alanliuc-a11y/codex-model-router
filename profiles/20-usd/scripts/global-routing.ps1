...
if (-not (Test-Path -LiteralPath $fragmentPath -PathType Leaf)) { throw "Missing routing fragment: $fragmentPath" }
$existing = if (Test-Path -LiteralPath $agentsPath -PathType Leaf) { Get-Content -LiteralPath $agentsPath -Raw -Encoding utf8 } else { '' }
$hasStart = $existing.Contains($startMarker)
$hasEnd = $existing.Contains($endMarker)
if ($hasStart -xor $hasEnd) { throw "Refusing to edit $agentsPath because its Model Router markers are incomplete. Repair the file manually first." }

if ($Disable) {
    if (-not $hasStart) { Write-Output "Model Router global workflow is not enabled in $agentsPath."; exit 0 }
    ...
    $updated = if ($hasStart) { [regex]::Replace($existing, $blockPattern, [System.Text.RegularExpressions.MatchEvaluator]{ param($match) $fragment }) } elseif ([string]::IsNullOrWhiteSpace($existing)) { $fragment + [Environment]::NewLine } else { $existing.TrimEnd() + [Environment]::NewLine + [Environment]::NewLine + $fragment + [Environment]::NewLine }
    $action = if ($hasStart) { 'update the managed Model Router block in' } else { 'add the managed Model Router block to' }
}

if ($Preview) { Write-Output "Would $action $agentsPath"; exit 0 }
